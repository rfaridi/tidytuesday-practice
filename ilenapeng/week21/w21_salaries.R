...
ggplot(data=df, aes(x=highest_edu, 
  ...
  labs(title="Breaking down US salaries under $300k by education and gender",
   ...
   theme_minimal() +
   theme(text=element_text(family="Roboto"),, axis.title=element_blank())
...
ggplot(data=df, aes(x=highest_edu, 
  ...
  labs(title="Breaking down US salaries under $300k by education and gender",
       ...
       caption="Data from Ask The Manager blog \n Graphic by Ilena Peng for #TidyTuesday") +
  theme_minimal() +
theme(text=element_text(family="Roboto"), axis.title=element_blank(), axis.text=element_text(color='Black'),panel.grid.major.x = element_line(color="white"), plot.title=element_text(size=20), plot.subtitle=element_markdown(size=16), plot.caption=element_text(size=8), plot.margin=margin(0.5,0.5,0.5,0.5, unit = "cm"))
